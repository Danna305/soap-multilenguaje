...
// javac -cp . version2_traduccion.java
// java -cp . version2_traduccion
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/", exchange -> {
            try {
                ...
                String urlTrad = "https://api.mymemory.translated.net/get?q="
                    + URLEncoder.encode(enIngles.trim(), "UTF-8") + "&langpair=en%7Ces";

                HttpRequest tradReq = HttpRequest.newBuilder()
                    .uri(URI.create(urlTrad))
                    .GET()
                    .build();

                HttpResponse<String> tradResp = client.send(tradReq, HttpResponse.BodyHandlers.ofString());
                String traducido = tradResp.body()
